...
describe RateBeer::Search do
  ...
  describe "#run_search" do
    ...
    it "returns a hash containing beers and breweries results" do
      expect(@failed_search).to include(:beers, :breweries)
      expect(@successful_search).to include(:beers, :breweries)
    end
